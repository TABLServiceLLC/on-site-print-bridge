...
-   Setup
    ...
    -   Generate HTTPS certs (self-signed CA + signed server cert) in the repo root:
        ...
        -   Create `server-ext.cnf` (kept alongside the CSR) so you can reuse the TLS extensions:
            ...
            subjectAltName=DNS:raspberrypi.local,IP:192.168.8.120
            EOF
            ```
...
-   Map a terminal to a printer - `curl -k -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
-d '{"terminalId":"t1","ip":"192.168.1.50"}' https://localhost:8443/assign`
-   Send a test print - `DATA=$(node -e "process.stdout.write(Buffer.from('Hello\\n','utf8').toString('base64'))")` - `curl -k -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
